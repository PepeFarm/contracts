...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // The FISH TOKEN!
    FishToken public pepe;
    address public devAddress;
    address public feeAddress;
    address public vaultAddress;

    // FISH tokens created per block.
    uint256 public fishPerBlock = 0.02 ether;
    ...
    uint16 public devCommissionRate = 100;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 devReward = fishReward.mul(devCommissionRate).div(10000);
        pepe.mint(devAddress, devReward);
        pepe.mint(address(this), fishReward.sub(devReward));
    ...
    // Safe pepe transfer function, just in case if rounding error causes pool to not have enough FISH.
    function safeFishTransfer(address _to, uint256 _amount) internal {
        uint256 fishBal = pepe.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > fishBal) {
            transferSuccess = pepe.transfer(_to, fishBal);
        } else {
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                pepe.mint(referrer, commissionAmount);
